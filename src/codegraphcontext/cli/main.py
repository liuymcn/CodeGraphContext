...
def config_init():
    """
    Generate a project-level .env configuration file in the current directory.

    Creates a commented template with all available settings and their defaults,
    making it easy to customize indexing behavior per project.

    Example:
        cd /path/to/your/project
        cgc config init
    """
    from .config_manager import DEFAULT_CONFIG, CONFIG_DESCRIPTIONS, DATABASE_CREDENTIAL_KEYS
    config_dir = Path.cwd() / ".codegraphcontext"
    config_dir.mkdir(exist_ok=True)
    env_path = config_dir / "config.env"

    if env_path.exists():
        if not typer.confirm(f"Config already exists at {env_path}. Overwrite?", default=False):
            console.print("[yellow]Cancelled[/yellow]")
            raise typer.Exit()

    lines = ["# CodeGraphContext - Project Configuration"]
    lines.append("# Generated by 'cgc config init'")
    lines.append("# Uncomment and modify settings as needed")
    lines.append("")
    ...
    env_path.write_text("\n".join(lines))
    console.print(f"[green]✔ Created project configuration at {env_path}[/green]")

    # Also generate .cgcignore if not present
    cgcignore_path = Path.cwd() / ".cgcignore"
    if not cgcignore_path.exists():
        from ..tools.graph_builder import DEFAULT_IGNORE_PATTERNS
        ignore_content = "# CodeGraphContext ignore patterns\n"
        ignore_content += "# Syntax: same as .gitignore\n\n"
        ignore_content += "\n".join(DEFAULT_IGNORE_PATTERNS) + "\n"
        cgcignore_path.write_text(ignore_content)
        console.print(f"[green]✔ Created {cgcignore_path}[/green]")

    console.print("[dim]Edit the files and customize as needed.[/dim]")
